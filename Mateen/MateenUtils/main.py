...
def load_model(load_mode, input_shape, scenario, train_loader, data, num_epochs):
    # Always create new model for now (force new model creation)
    print(f"Creating new autoencoder model with input shape: {input_shape}")
...
def adaptive_ensemble(x_train, y_train, x_slice, y_slice, args):
    cade_model = None
    model = ensemble_training(x_train, y_train=y_train, num_epochs=100, mode="init", scenario=args.dataset_name, load_mode="new")
